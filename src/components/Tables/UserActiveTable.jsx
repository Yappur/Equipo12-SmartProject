            ...
            {loading ? (
                ...
                <>
                    <div className="flex justify-between items-center mb-4">
                        ...
                        <Link to="/admin/panelUsuarios" className="text-sm text-gray-500 hover:underline">
                            Ver todos
                        </Link>
                    </div>
                    <div className="border-t border-gray-200">
                        {usersActive.map((user) => (
                            <div key={user.id} className="flex items-center py-4 border-b border-gray-200">
                                ...
                                <div className="flex-grow">
                                    <div className="font-medium text-[#00254B]">{user.displayName}</div>
                                </div>
                                <div className="flex-shrink-0 w-30 text-black capitalize">{user.role}</div>
                                <div className="flex-shrink-0 w-16 text-black">{formatMonthYear(user.createdAt)}</div>
                                <div className="flex-shrink-0 ml-4">
                                    <button className="flex items-center px-3 py-1 rounded-full">
                                        <div
                                            className={`px-4 py-1 rounded-full text-sm font-medium cursor-pointer ${user.estado === "Activo"
                                        ...
                                        >
                                            {user.estado} <FaChevronDown className="inline ml-1 text-xs" />
                                        </div>
                                    </button>
                                </div>
                            </div>
                        ...
                        <Modal
                            ...
                            mensaje={`¿Estás seguro de cambiar el estado de ${selectedUser?.displayName || ""
                                } a ${newStatus}?`}
